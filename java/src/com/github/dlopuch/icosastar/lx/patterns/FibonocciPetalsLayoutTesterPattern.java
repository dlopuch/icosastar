...
public class FibonocciPetalsLayoutTesterPattern extends LXPattern {
  ...
  public FibonocciPetalsLayoutTesterPattern(LX lx) {
    ...
    ledSelect = new DiscreteParameter("LED", -1, lx.model.points.size());

    portSelect = new DiscreteParameter("Port", -1, (int)Math.floor(lx.model.points.size() / LEDS_PER_PORT));
    portSelect.addListener(portSelect -> {
      if (portSelect.getValue() < 0) {
        ledSelect.setRange(-1, lx.model.points.size());
      } else {
